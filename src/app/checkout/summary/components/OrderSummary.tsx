...
const OrderSummary = ({ amount, totalItems }: OrderSummaryProps) => {
    return (
        <div className='flex flex-col gap-4 rounded-sm border p-4 text-lg font-semibold shadow-md'>
            ...
            <div className='flex flex-row justify-between'>
                <p className='ml-2 text-sm font-normal text-[#828282]'>Total items <span>({totalItems})</span></p>
                <div>
            ...
            <div className='flex flex-row justify-between'>
                ...
                <div>
                    <p className='font-meduim ml-2 text-sm text-black'>GH¢{amount.toFixed(2)}</p>
                </div>
            </div>
        </div>
    )
