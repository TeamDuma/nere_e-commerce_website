...
export const FAQ = () => {
  return (
    <section className='bg-[#FEFDFB]'>
      <div className='nere-container pb-[281px]'>
        ...
        <div className='grid gap-x-[86px] gap-y-[14px] md:grid-cols-2 md:gap-y-[30px]'>
          {faqs.map(({ question, answer }, index) => (
            <Disclosure key={index} as='div' defaultOpen={true}>
              <DisclosureButton className='flex w-full items-center justify-between rounded-md border-[0.5px] border-[#04484D33] bg-white px-4 py-6'>
                <span className='text-[18px] leading-[29.26px]'>
                  {question}
                </span>
                <PlusIcon />
              </DisclosureButton>
